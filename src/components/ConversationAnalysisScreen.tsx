...
const RADAR_LABELS = [
  { text: 'Communication', angle: -90 },
  { text: 'Problem Solving', angle: -30 },
  { text: 'Roles', angle: 30 },
  { text: 'Affective Responsiveness', angle: 90 },
  { text: 'Affective Involvement', angle: 150 },
  { text: 'Behavior Control', angle: 210 },
] as const
...
function RadarChart({
  ...
  const polygonPoints = values
    .map((v, i) => {
      const { x, y } = toXY(maxR * (Math.max(0, Math.min(100, v)) / 100), RADAR_LABELS[i]!.angle)
      return `${x.toFixed(1)},${y.toFixed(1)}`
    })
    .join(' ')

  return (
    <div className="relative mx-auto h-[292px] w-full max-w-[335px]">
      {/* Axis labels (HTML — avoids SVG clipping / overflow) */}
      <span className="pointer-events-none absolute left-1/2 top-0 max-w-[120px] -translate-x-1/2 text-center text-[10px] font-semibold leading-tight text-white/35">
        Communication
      </span>
      <span className="pointer-events-none absolute right-0 top-[46px] max-w-[78px] text-right text-[10px] font-semibold leading-tight text-white/35">
        Problem Solving
      </span>
      <span className="pointer-events-none absolute right-0 top-[118px] max-w-[52px] text-right text-[10px] font-semibold leading-tight text-white/35">
        Roles
      </span>
      <span className="pointer-events-none absolute bottom-0 left-1/2 max-w-[120px] -translate-x-1/2 text-center text-[10px] font-semibold leading-tight text-white/35">
        Affective Responsiveness
      </span>
      <span className="pointer-events-none absolute bottom-[34px] left-0 max-w-[88px] text-left text-[10px] font-semibold leading-tight text-white/35">
        Affective Involvement
      </span>
      <span className="pointer-events-none absolute left-0 top-[118px] max-w-[78px] text-left text-[10px] font-semibold leading-tight text-white/35">
        Behavior Control
      </span>
      ...
      >
        <defs>
          <linearGradient id="luxRadarFill" x1="0" y1="0" x2="0" y2="1">
        ...
        {gridSteps.map((s) => {
          const gridPoints = RADAR_LABELS.map((l) => {
            const { x, y } = toXY(maxR * s, l.angle)
            return `${x.toFixed(1)},${y.toFixed(1)}`
          }).join(' ')
          return (
            <polygon
        ...
        {RADAR_LABELS.map((l, idx) => {
          const { x, y } = toXY(maxR, l.angle)
          return (
            <line
              key={l.text + idx}
              x1={cx}
